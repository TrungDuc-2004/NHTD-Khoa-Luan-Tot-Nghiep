...
OPTIONAL_HEADERS: List[str] = ["lesson_url"]
...
def validate_headers(headers: List[str]) -> None:
    ...
    allowed = set(REQUIRED_HEADERS + OPTIONAL_HEADERS)
    extras = [h for h in headers if h not in allowed]
    if extras:
        print(f"[WARN] CSV có cột không dùng: {', '.join(extras)}")
    ...
    res = col.update_one(
        {"lesson_id": item["lesson_id"]},
        {
            "$set": {
                ...
                **({"lesson_url": item["lesson_url"]} if "lesson_url" in item else {}),
                "updatedAt": now,
            },
...
def import_csv(csv_path: str, mongo_uri: str, db_name: str, dry_run: bool = False):
        ...
        for i, row in enumerate(reader, 1):
            try:
                ...
                if action == "INSERT":
                    ...
                    upd += 1
                print(f"[{i}] {action} lesson_id='{item['lesson_id']}'" + ("" if dry_run else f" -> _id={oid}"))
    ...
    if dry_run:
        print("[NOTE] Chạy chế độ dry-run: không ghi dữ liệu vào DB.")

def parse_args():
    p = argparse.ArgumentParser(description="Import CSV 'lessons' vào MongoDB (upsert theo lesson_id)")
    p.add_argument("--csv", required=True, help="Đường dẫn CSV, ví dụ: csv/lesson.csv")
    p.add_argument("--mongo_uri", default=DEFAULT_MONGO_URI, help="Mongo URI (mặc định localhost)")
    p.add_argument("--db", default=DEFAULT_DB_NAME, help="Tên database (mặc định KLTN)")
    p.add_argument("--dry_run", action="store_true", help="Chạy thử: không ghi DB, chỉ log INSERT/UPDATE")
    return p.parse_args()
